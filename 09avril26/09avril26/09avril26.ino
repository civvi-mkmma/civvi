...
// ===== CONFIGURATION CIVVI =====
String monNomReseau = "Civvi-bt-mathieu-s3"; // Nom qui apparaitra en Bluetooth !
...
void envoyerJsonBLE() {
  ...
  // Pour éviter la saturation du tampon Bluetooth (Maximum Transmission Unit), on compresse par toutes petites briques.
  // 20 octets est la norme de base stricte sur TOUS les vieux et nouveaux appareils. 
  // Avec un délai de 30ms, on donne le temps à l'antenne d'émettre (sinon la puce panique et se déconnecte).
  for(int i = 0; i < json.length(); i += 20) {
    if(!deviceConnected) break; // Si on se déconnecte pendant l'envoi, on arrête
